...
async function sendRitaMsg() {
  ...
  try {
    var response = await fetch('https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash:generateContent?key=AIzaSyCCTiePInuZmDadlRtSNgaB_kjMZcEaGoQ', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        contents: [{
          parts: [{
            text: 'Você é o Neurinho 🧠, um assistente de estudos amigável do Sexta-Feira Studies. Você é um cérebro falante que ajuda alunos com dúvidas, explica matérias, dá dicas de estudo e é muito paciente. Use português brasileiro, emojis e seja animado!\n\nUsuário: ' + msg + '\n\nNeurinho:'
          }]
        }]
      })
    });
    ...
    var reply = data?.candidates?.[0]?.content?.parts?.[0]?.text || 'Desculpe, não entendi! 😅';
    ...
    var typingEl = document.getElementById(typingId);
    if (typingEl) typingEl.remove();
    messagesDiv.innerHTML += '<div style="text-align:left;margin-bottom:10px;"><div style="display:inline-block;max-width:80%;padding:10px 14px;border-radius:18px;background:#fee2e2;color:#991b1b;font-size:14px;">🧠❌ Ops! Erro: ' + (err.message || 'desconhecido') + '</div></div>';
  }
    ...
    if (msgLower.includes('oi') || msgLower.includes('olá') || msgLower.includes('ola')) {
      ...
      resposta = '🧠 Hmm, interessante! Me conte mais sobre isso. Estou aqui para ajudar você a aprender! 📚✨';
    }
